...
La qualité de l'analyse peut se dégrager significativement sur des textes longs.
La définition de ce qu'est un texte long dépend du contexte d'usage 
Dans les conditions de test cela correspondait à des textes d'environ 1500 mots.
...
───────────
  ...
  python C_rst_segment_local.py --fichier mon_passage.txt
  python C_rst_segment_local.py --no-confirm
"""
...
def generer_rapport_md(
    ...
    lignes += [
        "", "---", "",
        "*Rapport généré par `C_rst_segment_local.py`.*  ",
...
def main() -> None:
    ...
    print(f"\n  Analyse RST en cours ({LLM_MODEL}, T°={TEMPERATURE})…")
    try:
    ...
    print(f"\n   Lancez D_synthese_croisee_local.py pour l'analyse intégrée A+B+C.\n")
