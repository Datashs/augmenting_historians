...
La qualité de l'analyse peut se dégrager significativement sur des passages 
longs. Une limite, empirique, pourrait être de ne pas soumettre au dispositif
des passages de plus de 1500 mots.
...
───────────
  ...
  python B_perelman_segment_local.py --fichier mon_passage.txt
  python B_perelman_segment_local.py --no-confirm
"""
...
def extraire_scores(raw: str) -> dict:
    ...
    Quatre niveaux de robustesse (identique à A_toulmin_segment_local.py) :
...
def generer_rapport_md(
    ...
    lignes += [
        "", "---", "",
        "*Rapport généré par `B_perelman_segment_local.py`.*  ",
...
def main() -> None:
    ...
    try:
        raw = appeler_ollama(SYSTEM_PROMPT, construire_prompt(titre, texte, question))
    except Exception as e:
    ...
    print(f"\n   Lancez C_rst_segment_local.py sur le même segment,")
    print(f"   puis D_synthese_croisee_local.py pour l'analyse intégrée.\n")
