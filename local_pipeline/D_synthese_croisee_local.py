...
UTILISATION
───────────
  python D_synthese_croisee_local.py
  python D_synthese_croisee_local.py --toulmin resultats/toulmin_X.json \\
                                --perelman resultats/perelman_seg_X.json \\
                                --rst resultats/rst_X.json
  python D_synthese_croisee_local.py --no-confirm
"""
...
def generer_rapport_md(
    ...
    lignes += [
        "",
        "*Rapport généré par `D_synthese_croisee_local.py`.*  ",
...
def confirmer_generation(tokens: int, no_confirm: bool) -> None:
    ...
    print(f"\n  Estimation : ~{tokens} tokens | ~{temps_est}s ({LLM_MODEL})")
    if no_confirm:
        return
...
def main() -> None:
    ...
    parser.add_argument("--toulmin",  type=str, default=None,
                        help="JSON produit par A_toulmin_segment_local.py.")
    parser.add_argument("--perelman", type=str, default=None,
                        help="JSON produit par B_perelman_segment_local.py.")
    parser.add_argument("--rst",      type=str, default=None,
                        help="JSON produit par C_rst_segment_local.py.")
    ...
    temps_est = (tokens_in + MAX_TOKENS) // 10
    print(f"\n  Estimation : ~{tokens_in} tokens | ~{temps_est}s ({LLM_MODEL})")
    ...
    print(f"\n  Synthèse croisée en cours ({LLM_MODEL})…")
    try:
