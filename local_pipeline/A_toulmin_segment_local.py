...
Ce script-ci opère différemment :
  - L'historien délimite lui-même le segment à analyser (quelques paragraphes,
    une section, un développement complet) en gardant à l'esprit que la qualité 
    de l'anlyse peut se dégrader de manière significative sur de trop longs 
    segments (1500 mots est sans doute une limite raisonnable).
...
─────────────────
  ...
  WALTON (1996) — Argumentation Schemes for Presumptive Reasoning, Erlbaum.
    ...
    Le script le signale sans le condamner.

POLYPHONIE — LIMITE IMPORTANTE
────────────────────────────────
...
───────────
  ...
  python A_toulmin_segment_local.py --fichier mon_passage.txt
  python A_toulmin_segment_local.py --no-confirm   (sans demande de confirmation coût)
"""
...
# LLM_MODEL et OLLAMA_URL importés depuis rag_config_local.py.
# Pour changer de modèle, modifier LLM_MODEL dans rag_config_local.py.
# Recommandation : deepseek-r1:14b ou gemma3:12b pour les analyses
# argumentatives (meilleure représentation de Toulmin/Perelman/RST).
...
# Dossier de sortie pour les rapports MD et JSON
OUTPUT_DIR = "outputs"
...
def estimer_tokens(texte: str, titre: str, question: str) -> int:
    """
    Estime le nombre de tokens du prompt (approximation 4 chars/token).
    En local, pas de coût financier — on affiche juste le volume
    pour anticiper le temps de génération.
    """
    chars = len(SYSTEM_PROMPT) + len(texte) + len(titre) + len(question) + 500
    return chars // 4


def confirmer_generation(tokens: int, no_confirm: bool) -> None:
    """Affiche une estimation du temps avant de lancer Ollama."""
    # Estimation grossière : ~10 tokens/seconde sur M2 14B
    temps_est = (tokens + MAX_TOKENS) // 10
    print(f"\n  Estimation : ~{tokens} tokens en entrée | ~{temps_est}s de génération ({LLM_MODEL})")
    if no_confirm:
        return
...
def generer_rapport_md(
    ...
    lignes += [
        "", "---", "",
        "*Rapport généré par `A_toulmin_segment_local.py`.*  ",
...
def appeler_ollama(system_prompt: str, user_prompt: str) -> str:
    """
    Soumet un prompt à Ollama et retourne la réponse complète.

    Combine system_prompt et user_prompt en un seul prompt
    (Ollama /api/generate ne distingue pas les rôles comme OpenAI).
    Timeout généreux (300s) pour les analyses longues sur CPU/M2.
...
def main() -> None:
    ...
    # ── Estimation coût ───────────────────────────────────────────────────────
    tokens = estimer_tokens(texte, titre, question)
    confirmer_generation(tokens, args.no_confirm)

    # ── Appel LLM ─────────────────────────────────────────────────────────────
    print(f"\n  Analyse en cours ({LLM_MODEL}, T°={TEMPERATURE})…")
    prompt = construire_prompt(titre, texte, question)
    try:
        raw = appeler_ollama(SYSTEM_PROMPT, prompt)
    except Exception as e:
    ...
    print(f"\n   Lancez B_perelman_segment_local.py et C_rst_segment_local.py")
